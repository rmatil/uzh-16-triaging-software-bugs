...
REPUTATION_RATE = (
    'SELECT (nr_closed*1.0/(nr_closed+nr_failed)) AS reputationRate, reports.bug_id '
    'FROM ( '
    'SELECT reputation.reporter, ifnull(nr_closed, 0) AS nr_closed, ifnull(nr_failed, 0) AS nr_failed '
    'FROM(  '
    'SELECT closed.reporter, closed.nr_closed, failed.reporter, failed.nr_failed '
    'FROM ( '
    ...
    ') AS reputation '
    ') AS repRate '
    'INNER JOIN reports ON reports.reporter = repRate.reporter ORDER BY reports.bug_id;'
)
