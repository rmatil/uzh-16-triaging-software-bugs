...
REOPENINGS = ('SELECT '
              ...
              'FROM reports '
              'LEFT JOIN (SELECT '
              'COUNT(bug_status.id) AS count, '
              'bug_status.bug_id '
              'FROM bug_status '
              'WHERE bug_status.what = "REOPENED" '
              'GROUP BY bug_id '
              'ORDER BY count '
              'DESC) AS rate ON rate.bug_id = reports.bug_id '
              'ORDER BY reports.bug_id;'
              )
...
OPEN_TIME = ('SELECT (minmax.max - minmax.min) AS duration, reports.bug_id '
             ...
             'current_status == "CLOSED" AND reports.current_resolution == "FIXED" '
             'UNION '
             'SELECT (minmax.max - minmax.min) AS duration, reports.bug_id '
             'FROM ( SELECT bug_id, MIN(timestamp) AS min, MAX(timestamp) AS max '
             'FROM resolution '
             'GROUP BY bug_id '
             'ORDER BY bug_id, timestamp '
...
# 11. success-rate related to the priority of a bug

PRIORITYRATE = ('SELECT result.successRate, rep.bug_id '
                'FROM ( SELECT reports.bug_id, priority.what AS what '
                'FROM priority '
                'JOIN reports ON priority.bug_id = reports.bug_id) rep '
                'JOIN ( '
                'SELECT what, (successCount*1.0/(successCount+failCount)) AS successRate '
                'FROM (SELECT ifnull(success.what, fail.what) AS what, ifnull(success.count, 0) AS successCount, ifnull(fail.count, 0) AS failCount '
                'FROM ( SELECT priority.what AS what, count(reports.bug_id) AS count '
                'FROM priority '
                'INNER JOIN reports ON priority.bug_id = reports.bug_id '
                'WHERE current_status == "RESOLVED" AND reports.current_resolution == "WORKSFORME" OR '
                'current_status == "RESOLVED" AND reports.current_resolution == "FIXED" OR '
                'current_status == "VERIFIED" AND reports.current_resolution == "FIXED" OR '
                'current_status == "CLOSED" AND reports.current_resolution == "WORKSFORME" OR '
                'current_status == "CLOSED" AND reports.current_resolution == "FIXED" '
                'GROUP BY priority.what '
                ')  AS success '
                'LEFT OUTER JOIN ( '
                'SELECT priority.what AS what, count(reports.bug_id) AS count '
                'FROM priority '
                'INNER JOIN reports ON priority.bug_id = reports.bug_id '
                'WHERE current_status == "RESOLVED" AND reports.current_resolution == "WONTFIX" OR '
                'current_status == "RESOLVED" AND reports.current_resolution == "INVALID" OR '
