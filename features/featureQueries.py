# 1. success-rate
SUCCESS_RATE = ('SELECT success.bug_id, success.count*1.0 / (success.count + fail.count) AS feature_1 '
                'FROM (SELECT reports.bug_id, assigned_to.who, count(assigned_to.who) AS count '
                'FROM reports '
...
REPUTATION_RATE = ('SELECT reporter, nr_closed, nr_failed, (nr_closed*1.0/(nr_closed+nr_failed)) AS reputationRate '
                   'FROM ( '
...
REASSINGMENTS = (
    'SELECT s1.nr_assignments, s1.nrSuccessReassignments, f1.nrFailedReasignments, (s1.nrSuccessReassignments*1.0/(s1.nrSuccessReassignments+f1.nrFailedReasignments)) AS bugSuccessRate'
